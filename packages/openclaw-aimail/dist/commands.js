 ...
 */
export async function registerAgentEmail(client, opts) {
    const result = await client.request('POST', `/api/v1/admin/systems/${opts.systemId}/addresses?generate_code=true`, {
    ...
    if (status && !['created', '200', '201'].includes(status)) {
        ...
        if (/already exists|exists/i.test(msg)) {
            ...
            const domains = await client.request('GET', `/api/v1/admin/systems/${opts.systemId}/domains`);
            ...
            for (const d of entries) {
                ...
                if (row.domain === opts.email) {
                    await client.request('PUT', `/api/v1/admin/system-domains/${String(row.id)}`, {
                        webhook_url: opts.webhookUrl,
                        webhook_secret: opts.webhookSecret,
                    });
                    break;
    ...
    if (!activationCode)
        return {};
    const act = await client.request('POST', '/api/v1/activate-address', { code: activationCode, email_address: opts.email });
    const apiKey = act.raw_key;
    return { api_key: apiKey, activation_code: activationCode };
}
/** 3-step idempotent deregistration chain (api-key → domain → whitelist). */
export async function deregisterAgentEmail(client, opts) {
    const out = { api_key: '', domain: '', whitelist: '' };
    // 1. API key by email → delete
    try {
        const k = await client.request('GET', `/api/v1/admin/api-keys?email=${encodeURIComponent(opts.email)}`);
        const entries = Array.isArray(k.data)
            ? k.data
            : k.entries ?? [];
        const key = entries[0];
        if (key && key.id) {
            const r = await client.request('DELETE', `/api/v1/admin/api-keys/${String(key.id)}`);
    ...
    try {
        ...
        if (addrId) {
            const r = await client.request('DELETE', `/api/v1/admin/system-domains/${addrId}`);
    ...
    try {
        ...
        const r = await client.request('DELETE', `/api/v1/whitelists?${q.toString()}`);
...
async function resolveSystemId(explicit) {
    if (explicit)
        return explicit;
    const ptr = await readPointer();
    if (ptr.system_id)
        return ptr.system_id;
    throw new Error('no system_id — pass --system-id or activate the agentmail pointer (~/.openclaw/.agentmail)');
}
async function readGatewayConfig(systemId) {
    const p = path.join(systemDir(systemId), 'agentmail_gateway.json');
    try {
        return JSON.parse(await fs.readFile(p, 'utf-8'));
    }
    catch {
        throw new Error(`gateway config not found (agentmail_gateway.json) for ${systemId} — activate first`);
    }
}
function saveAgentConfig(agentId, cfg, systemId) {
    const email = String(cfg.email ?? '');
    if (!email)
        throw new Error('saveAgentConfig requires email');
    const p = agentConfigPath(systemId, email);
    return fs.mkdir(path.dirname(p), { recursive: true }).then(() => fs.writeFile(p, JSON.stringify({ ...cfg, agent_id: agentId }, null, 2) + '\n'));
}
function cmdText(lines) {
...
async function handleCommand(ctx) {
    ...
    try {
        if (sub === 'register') {
            ...
            if (!email)
                return cmdText(['register requires --email <addr>', '', USAGE]);
            ...
            if (reg.api_key) {
                await saveAgentConfig(ctx.agentId ?? 'main', {
                    email,
                ...
                const ptrPath = path.join(process.env.HOME ?? process.env.USERPROFILE ?? '', '.openclaw', '.agentmail');
                await fs.mkdir(path.dirname(ptrPath), { recursive: true });
                await fs.writeFile(ptrPath, JSON.stringify({ system_id: systemId, email }, null, 2) + '\n');
                return cmdText([
        ...
        if (sub === 'deregister') {
            const email = opts.email ?? '';
            if (!email)
                return cmdText(['deregister requires --email <addr>', '', USAGE]);
            const systemId = await resolveSystemId(opts['system-id'] ?? '');
            const gw = await readGatewayConfig(systemId);
            const { GatewayClient } = await import('@aimail/mail-core');
...
export function createAimailCommands() {
    return [
        {
            name: 'aimail',
