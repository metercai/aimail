...
| `/api/v1/activate-address-code` | POST | Address-code → agent key (address-level self-service mailbox) | `pysdk/aimail_tools.py` (`activate_address_code`), `tssdk mail-core address-code.ts` (`activateAddressCode`/`activateAddressCodePersist`); **advanced-only** (aimail-advanced `src/advanced/api/address.rs`; flow documented in `docs/agent-self-setup.md`) |
...
| `/api/v1/whitelists`           | GET/POST   | List/create whitelist (agent: own; agent\_admin: scoped)            | No SDK caller (neither Python SDK nor TS mail-core exposes list/create client methods) → gateway/admin side                                                                                                                                                                         |
...
Pass the target email when operating on others.
...
| `/api/v1/admin/systems/:sid/addresses/rename` | POST | Rename a registered agent address (server-side resources fully inherited) | `cli/aimail` (`address` subcommand, `-n` set-name → `_rename_after_reg`) |
...
| `/api/v1/system/welcome`               | POST     | System welcome mail (server-fixed subject/body, sole param `to`; scope = platform/system/agent\_admin) | `cli/send_welcome.py` (`_api_send`, admin-key signed; `aimail welcome` goes through it) — the endpoint replacing the stats polling in welcome tests (base gateway `src/core/api/http.rs`)                            |
...
| `/api/v1/admin/renew-system`           | POST     | Renew a system with a product code (additive `max(now, expiry) + validity`)                            | `cli/aimail` (`renew -c <code>`); **advanced-only** (aimail-advanced `src/advanced/strategy.rs`; the base gateway has no such route)                                                                                  |
| `/api/v1/quotas`                       | GET      | Read-only system quota / expiry view                                                                   | `cli/aimail` (`renew --status`, `stats -a`); **advanced-only** (aimail-advanced `src/advanced/strategy.rs`)                                                                                                           |

## Bridge

Local aimail-bridge API.
...
Auth: board API credentials are per-member tokens (Bearer auth — not the API key). Credentials arrive via the invite mail —
Subject `[A2A] invite: {board}`, body carries the API URL + personal token — and the agent stores them locally for the SDK board calls.
...
| `/api/v1/board/:id/roles`               | GET    | List role permissions          | `pysdk/aimail_board.py` (`board_roles`) + `tssdk mail-core board.ts` (`boardRoles`, tool `board_roles`) |
