...
> Authoritative code: `pysdk/` (shared core + platform adapters + MCP server), `cli/` (CLI and scripts), `resources/skills/` at the repo root (SKILL sources; the per-package `resources/` copies are generated by `scripts/materialize-resources.sh`), `cli/bin/` (runtime registration tools).
...
| Install source | `resources/skills/SKILL.md` + `DESCRIPTION.md` (repo root; per-package copies generated) | Generic email skill (byte-exact copy, zero rewriting) |
