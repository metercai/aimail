...
| CLI 层 | `scripts/aimail`(10 子命令)+ `scripts/gateway_api.py` | 安装/检查/测试/卸载/运维 |
...
- `send_pong` 经 `_CONFIG_LOADER` 解析配置走 `send_mail`;日志统一 `~/.aimail/logs/agentmail.{cleaned_addr}.log`。
...
- 三阶段事件:`ping_intercepted → pong_sent → pong_returned`,落 `~/.aimail/logs/agentmail.{cleaned_addr}.log`(ping_test 唯一权威判定)。
...
| 部署 | 共享布局(~/.aimail/systems/{sid}/{cleaned_addr}/agentmail.json);入站补丁经 `scripts/deer-flow/install-inbound.sh` 幂等安装(拷贝 + app.py 双锚点 patch + py_compile 校验;上游仓保持干净,安装后重启 8001 生效) |
...
## 5. CLI 契约(scripts/aimail)

**命令名冲突警告**:`~/.local/bin/aimail` 是 Hermes 启动器,repo 的 CLI 只能经仓库根 `./aimail` 运行,不得把 scripts/ 加进全局 PATH。
...
- **integrate.sh / uninstall.sh / bridge-ctl.sh**:已被 `aimail install/uninstall/bridge` 取代。
- **agentmail_gateway.json 旧名**:统一 `agentmail_gateway.json`,无兼容别名。
