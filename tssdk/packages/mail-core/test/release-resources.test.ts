...
beforeAll(() => {
  ...
  const perDir: Record<string, number> = { role_prompt: 6 }
  for (const [sub, n] of Object.entries(perDir)) {
    const d = path.join(sdkBoard, sub)
    fs.mkdirSync(d, { recursive: true })
    for (let i = 0; i < n; i++) {
      fs.writeFileSync(path.join(d, i === 0 && sub === 'role_prompt' ? 'role_calibrator.md' : `file${i}.md`), `# ${sub} ${i}`)
    }
  }
})
...
describe('releaseResources', () => {
  test('releases the board role-prompt dir into the system config dir', () => {
    const r = releaseResources({ systemId: 'sys-a', boardRoot: sdkBoard })
    expect(r.copied).toBeGreaterThanOrEqual(6)
