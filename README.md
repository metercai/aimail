...
`dsh plugin add dsh-aimail` = profile 目录 pnpm add + bundle reconcile。不发布时用**本地路径安装**模拟全流程:
...
# 2. 模拟 dsh plugin add:
cd ~/.dsh/profiles/web
pnpm add /path/to/dsh-aimail/packages/agentmail
#    → dependencies 写入 link: 路径,node_modules/dsh-aimail 符号链接

# 3. 运行时加载验证:
node -e "import('dsh-aimail').then(async m => {
...
#    profile 的 dsh.profile.bundles(实测:dsh-aimail 加入,4 条目 patch 就位)

# 5. 模拟卸载(幂等):
pnpm remove dsh-aimail
#    → dependencies 清空 + bundles 移除 dsh-aimail
```
